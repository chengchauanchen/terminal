...
public class AnimationsContainer {
    ...
    public int FPS = 2;  // 每秒播放帧数，fps = 1/t，t-动画两帧时间间隔
    private int resId = R.array.sent_voice_anim; //图片资源
    private Context mContext;
    // 单例
    private static AnimationsContainer mInstance;


    public AnimationsContainer(Context mContext){
        this.mContext = mContext;
    }
    //获取单例
    public static AnimationsContainer getInstance(Context mContext, int resId, int fps) {
        if (mInstance == null){
            mInstance = new AnimationsContainer(mContext);
        }
