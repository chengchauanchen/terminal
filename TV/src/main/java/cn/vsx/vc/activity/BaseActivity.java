				...
				if (MyTerminalFactory.getSDK().getParam(Params.VOLUME_UP, false)) {
					...
					if(System.currentTimeMillis() - lastVolumeUpTime > 500) {
						...
						if(MyTerminalFactory.getSDK().getAudioProxy().getVolume()>0){
							TerminalFactory.getSDK().notifyReceiveHandler(ReceiveVolumeOffCallHandler.class, false,1);
						}else {
							TerminalFactory.getSDK().notifyReceiveHandler(ReceiveVolumeOffCallHandler.class, true,1);
						}
