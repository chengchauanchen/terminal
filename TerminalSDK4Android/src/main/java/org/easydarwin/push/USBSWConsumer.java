...
public class USBSWConsumer extends Thread implements USBVideoConsumer{
    private static final String TAG = "USBSWConsumer";
    private int mHeight;
    private int mWidth;
    ...
    public USBSWConsumer(Context context, Pusher pusher){
        mPusher = pusher;
    }
    @Override
    ...
    public void run(){
        ...
        do {
            try {
                int r = 0;
                USBSWConsumer.TimedBuffer tb = yuvs.take();
                byte[] data = tb.buffer;
    ...
    public int onVideo(byte[] data, int format) {
        try {
            ...
            yuvs.offer(new USBSWConsumer.TimedBuffer(buffer));
            if (time > 0) Thread.sleep(time / 2);
            lastPush = System.currentTimeMillis();
