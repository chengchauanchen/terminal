 ...
 */
public final class RTMPCodecSpecificDataUtil{
  ...
  private RTMPCodecSpecificDataUtil() {}
  ...
  public static Pair<Integer, Integer> parseAacAudioSpecificConfig(byte[] audioSpecificConfig) {
    RTMPParsableBitArray bitArray = new RTMPParsableBitArray(audioSpecificConfig);
  ...
  public static byte[] buildAacLcAudioSpecificConfig(int sampleRate, int numChannels) {
    int sampleRateIndex = RTMPC.INDEX_UNSET;
    for (int i = 0; i < AUDIO_SPECIFIC_CONFIG_SAMPLING_RATE_TABLE.length; ++i) {
      if (sampleRate == AUDIO_SPECIFIC_CONFIG_SAMPLING_RATE_TABLE[i]) {
        sampleRateIndex = i;
      }
    }
    int channelConfig = RTMPC.INDEX_UNSET;
    for (int i = 0; i < AUDIO_SPECIFIC_CONFIG_CHANNEL_COUNT_TABLE.length; ++i) {
      if (numChannels == AUDIO_SPECIFIC_CONFIG_CHANNEL_COUNT_TABLE[i]) {
        channelConfig = i;
      }
    }
    if (sampleRate == RTMPC.INDEX_UNSET || channelConfig == RTMPC.INDEX_UNSET) {
  ...
  public static byte[][] splitNalUnits(byte[] data) {
    ...
    } while (nalUnitIndex != RTMPC.INDEX_UNSET);
   ...
   * @return The index of the first byte of the found start code, or {@link RTMPC#INDEX_UNSET}.
   */
