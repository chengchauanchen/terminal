...
//            id = R.drawable.rc_ad_list_audio_icon;
     ...
     * @param callback
     */
    public static void loadImageForSDCard(final Context context, final DataCallback<ArrayList<Image>> callback) {
            ...
            public void run() {
                ...
                if (mCursor != null) {
                    while (mCursor.moveToNext()) {
                        ...
                        long size = mCursor.getLong(
                                mCursor.getColumnIndex(MediaStore.Images.Media.SIZE)
                        );
                        images.add(new Image(path, time, name, (size+"")));
                    }
                    mCursor.close();
                }
