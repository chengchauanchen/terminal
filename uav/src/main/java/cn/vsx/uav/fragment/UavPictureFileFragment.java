...
public class UavPictureFileFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener{
    ...
    public static UavPictureFileFragment newInstance(boolean showCheckbox){
         Bundle args = new Bundle();
        args.putBoolean("showCheckbox",showCheckbox);
         UavPictureFileFragment fragment = new UavPictureFileFragment();
...
//            getData(currentIndex);
    ...
    private synchronized void getNewData(int pageIndex){
        ...
        for(String fileDate : fileDates){
            boolean first = true;
            int realFileSize = 0;
            CopyOnWriteArrayList<BitStarFileRecord> bitStarFileRecords = TerminalFactory.getSDK().getSQLiteDBManager().getBitStarFileRecords(fileDate, FileTransgerUtil.TYPE_IMAGE);
